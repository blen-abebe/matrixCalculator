...
def get_matrix(index):
    while True:
        try:
            row = int(input(f"Enter the number of rows for {index} matrix: ").strip())
            col = int(input(f"Enter the number of columns for {index} matrix: ").strip())
            ...
            while True:
                ...
                if any(len(matrix[x]) != col for x in range(row)):
            ...
            print("Please enter a valid number for rows and columns.")

# Collect matrices separately
print("Matrix 1:")
rows1, cols1, matrix1 = get_matrix("first")

print("\nMatrix 2:")
rows2, cols2, matrix2 = get_matrix("second")
